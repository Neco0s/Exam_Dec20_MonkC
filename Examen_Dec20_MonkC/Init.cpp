...
{
	int occupied = 1;
	while (occupied) //Boucle tant que le pisteur n'est pas à un endroit correct
	{
		occupied = 0;
		...
		if (pisteurs[index].x > 0 && pisteurs[index].x < map_width - 1 && pisteurs[index].y > 0 && pisteurs[index].y < map_height - 1) //Vérification de l'existence de l'emplacement
		{
			for (int i = 0; i < index; i++)
				if (pisteurs[index].x == pisteurs[i].x && pisteurs[index].y == pisteurs[i].y) //Vérification de la disponibilité de l'emplacement
				{
					occupied = 1;
...
{
	int occupied = 1;
	while (occupied) //Boucle tant que le monk n'est pas à un endroit correct
	{
		occupied = 0;
		...
		for (int i = 0; i < 3; i++) //axe Y
			for (int j = 0; j < 3; j++) //axe X
				for (int k = 0; k < n_pisteurs; k++) //pour chaque pisteur
					if (monk->y + i - 1 == pisteurs[k].y && monk->x + j - 1 == pisteurs[k].x) //Vérification de la proximité de l'emplacement avec les pisteurs
						occupied = 1;
	}
