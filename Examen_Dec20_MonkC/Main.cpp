...
//ENTREES UTILISATEUR: OUI
{
	...
	while (play)
	{
		...
		t_img* images = (t_img*)calloc(2 + n_pisteurs, sizeof(t_img)); //Création des sprites avec allocation de mémoire sécurisée
		if (images == NULL)
		{
		...
		printf("\nVoulez-vous rejouer?\nEntrez la valeur 1 pour rejouer, 0 pour quitter le jeu.\n");
		scanf("%d", &play);
